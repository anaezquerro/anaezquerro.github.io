+++
title = ""
+++

## Education 📚




**2019-2023**: Data Science and Engineering Degree at University of A Coruña.
- Honor distinction in the Final Degree Project: [*"Fully Incremental Parsing based on Neural Networks"*](https://ruc.udc.es/dspace/handle/2183/33269).

**2023-Present**: Master in Artificial Intelligence at University of A Coruña.


## Experience 💻
**07/2021-09/2021** (full-time): Research Assistant at [CITIC](https://citic.udc.es/en/home-english-2/).
- Analysis and comparison of cure models against ML techniques in COVID19 data for UCI admission prediction.

**09/2021-04/2022** (full-time): Department Collaboration Scholarship (University of A Coruña, [MODES](https://dm.udc.es/modes/) Group).
- New method for automatic covariates selection in dynamic regression models for time series analysis.

**09/2022-10/2022** (part-time): Student Scholarship at [Galician Institute in Technology](https://itg.es/en/).
- NLP with ASR tasks for chatbot designing with audio support.

**02/2023-09/2023** (part-time): Student Internship at [ABANCA](https://www.abanca.com/es/).
- Building and analysis of ML models for the estimation of commercial activities in the Customer Intelligence area.

**07/2023-09/2023** (part-time): Research Assistant at [CITIC](https://citic.udc.es/en/home-english-2/).
- Implementation and evaluation of supervised models for constituency parsing.

**10/2023-Present** (part-time): Research Support Technician at  [CITIC](https://citic.udc.es/en/home-english-2/).
- Development of new algorithms for parsing in NLP.
- Analysis of the viability of incremental representations for parsing.

## Additional positions

**2020-2023**: Compensation Tribunal member of the Computer Science Faculty at University of A Coruña.

**2022-Present**: Active member in the (1) Student Board of the Computer Science Faculty at University of A Coruña, (2) Master in Artificial Intelligence Committee and (3) Docentia's Program Tribunal.