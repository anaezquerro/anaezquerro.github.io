# Hi👋 I am **Ana Ezquerro**.

I'm an NLP researcher at the [LyS Group](https://www.grupolys.org/) ([University of A Coruña](https://www.udc.es)). I previously obtained a bachelor degree in [Data Science and Engineering](https://estudos.udc.es/en/study/detail/614g02v01) (July 2023), and recently a master degree in [Artificial Intelligence](https://mia.udc.es/) (February 2025).

My initial research covers **time series** analysis and **machine learning** algorithms applied to **statistics** and right now I am working on **graph/tree (incremental) parsing** and **multimodal learning**. I am also planning a PhD degree on **efficient and human-like incremental language representations**. 


## News

- **Feb 21, 2025**: Completed my [Master's degree in Artificial Intelligence (UDC)](https://mia.udc.es/) with an honor distinction for my thesis on *Graph Parsing as Sequence Labeling*.
- **Dec 9, 2024**: Paper accepted a [NoDaLiDa-2025](https://www.nodalida-bhlt2025.eu/)!
- **Nov 19, 2024**: If you have approached me in our poster session at [EMNLP 2024](https://2024.emnlp.org/) (or you did not have time to do it), feel free to ask any doubt about our work😄.
- **Nov 6, 2024**: Headed to Miami🏖️ to present our graph linearizations at [EMNLP 2024](https://2024.emnlp.org/)!

## Research 

- **Better Benchmarking LLMs for Zero-Shot Dependency Parsing** (to appear)

    [Ana Ezquerro](https://anaezquerro.github.io), [David Vilares](https://www.grupolys.org/~david.vilares/) and  [Carlos Gómez-Rodríguez](https://www.grupolys.org/~cgomezr/)

    *In the Joint 25th Nordic Conference on Computational Linguistics and 11th Baltic Conference on Human Language Technologies (NoDaLiDa/Baltic-HLT 2025)*.

    [poster](https://anaezquerro.github.io/material/nodalida-2025-poster.pdf)

- **Dependency Graph Parsing as Sequence Labeling** 

    [Ana Ezquerro](https://anaezquerro.github.io), [David Vilares](https://www.grupolys.org/~david.vilares/) and  [Carlos Gómez-Rodríguez](https://www.grupolys.org/~cgomezr/)

    *In the 2024 Conference on Empirical Methods in Natural Language Processing (EMNLP 2024)*

    [pdf](https://aclanthology.org/2024.emnlp-main.659/)  |  [code](https://github.com/anaezquerro/separ)  |  [slides](https://anaezquerro.github.io/material/emnlp-2024-slides.pdf)  |  [poster](https://anaezquerro.github.io/material/emnlp-2024-poster.pdf)


- **LyS at SemEval 2024 Task 3. An Early Prototype for End‑to‑End Multimodal Emotion Linking as Graph‑Based Parsing**

    [Ana Ezquerro](https://anaezquerro.github.io) and [David Vilares](https://www.grupolys.org/~david.vilares/)

    *In the 18th International Workshop on Semantic Evaluation (SemEval 2024) co‑located with the 2024 Annual Conference of the North American Chapter of the Association for Computational Linguistics (NACL 2024)*
    
    [pdf](https://aclanthology.org/2024.semeval-1.182/)  |  [code](https://github.com/anaezquerro/semeval24-task3)


- **From Partial to Strictly Incremental Constituent Parsing** 

    [Ana Ezquerro](https://anaezquerro.github.io), [Carlos Gómez-Rodríguez](https://www.grupolys.org/~cgomezr/) and [David Vilares](https://www.grupolys.org/~david.vilares/).
    
    *In the 18th Conference of the European Chapter of the Association for Computational Linguistics (EACL 2024)*
    
    [pdf](https://aclanthology.org/2024.eacl-short.21/)  |  [code](https://github.com/anaezquerro/incpar)  |  [slides](https://anaezquerro.github.io/material/eacl-2024.pdf)

- **On the Reliability of Machine Learning Models for Survival Analysis When Cure Is a Possibility**

    [Ana Ezquerro](https://anaezquerro.github.io), [Brais Cancela](https://pdi.udc.es/es/File/Pdi/9Y9SH) and [Ana López-Cheda](https://pdi.udc.es/es/File/Pdi/GJ85H)

    *In Mathematics Vol. 11, Nonparametric Statistical Methods and their Applications*

    [pdf](https://www.mdpi.com/2227-7390/11/19/4150)


- **On the Challenges of Fully Incremental Neural Dependency Parsing** 

    [Ana Ezquerro](https://anaezquerro.github.io), [Carlos Gómez-Rodríguez](https://www.grupolys.org/~cgomezr/) and [David Vilares](https://www.grupolys.org/~david.vilares/)
    
    *In the 13th International Joint Conference on Natural Language Processing and the 3rd Conference of the Asia-Pacific Chapter of the Association for Computational Linguistics (IJCNLP-AACL 2023)*

    [pdf](https://aclanthology.org/2023.ijcnlp-short.7/)  |  [code](https://github.com/anaezquerro/incpar)  |  [poster](https://anaezquerro.github.io/material/aacl-2023.pdf)


- **Automatic covariates selection in dynamic regression models with application to COVID-19 evolution**
    
    [Ana Ezquerro](https://anaezquerro.github.io), [Germán Aneiros-Pérez](https://dm.udc.es/staff/german_aneiros/) and [Manuel Oviedo](https://pdi.udc.es/gl/File/Pdi/3K3FM)
    
    *In the 5th XoveTIC Conference (XoveTIC 2022)*
    
    [pdf](https://easychair.org/publications/paper/1gRq)  |  [code](https://github.com/anaezquerro/dynamic-arimax)  |  [slides](https://github.com/anaezquerro/dynamic-arimax/blob/main/proceedings/presentation.pdf)


